...
import { syncWebsite, SyncServiceError } from "@/lib/integrations/sync-service";
...
export async function POST(request: Request) {
  ...
  try {
    const result = await syncWebsite(authorization.session.user.id);
    await writeAuditLog({
    ...
    return NextResponse.json({ error: "Sinkronisasi API website MAN 1 gagal." }, { status: 500 });
  }
}
